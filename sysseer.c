...
static void handle_dashboard(SOCKET s) {
    const char* html =
        ...
        "h1{font-size:1.4rem;color:#58a6ff;margin-bottom:16px;display:inline}"
        ".live{font-size:0.7rem;color:#3fb950;margin-left:8px}"
        ".row{display:flex;justify-content:space-between;padding:6px 0;border-bottom:1px solid #21262d}"
        ".label{color:#8b949e}.value{color:#c9d1d9;font-weight:500;text-align:right}"
        ".footer{font-size:0.75rem;color:#484f58;text-align:center;margin-top:8px}"
        "</style>"
        "</head><body>"
        "<h1>sysseer</h1><span class=live>LIVE</span>"
        "<div id=box class=card>loading</div>"
        "<div class=footer>last update: <span id=ts></span></div>"
        "<script>"
        "function upd(){"
        "document.getElementById('ts').textContent=new Date().toLocaleTimeString();"
        "fetch('/status',{cache:'no-store'}).then(r=>r.text()).then(t=>{"
        "let lines=t.split('\\n').filter(l=>l.trim());"
        "let h=lines.map(l=>{"
        "let p=l.split(/\\s{2,}/);"
        "if(p.length<2) return '<div class=row><span>'+l+'</span></div>';"
        "return '<div class=row><span class=label>'+p[0]+'</span><span class=value>'+p.slice(1).join(' ')+'</span></div>'"
        "}).join('');"
